...
def main():
    ...
    for i, fn in enumerate(FOLDER_NAMES):
        ...
        axs[i].plot(NUM_ELE, E12_errs, label=r"$E^{(12)}$", marker=".")
        axs[i].plot(NUM_ELE, E23_errs, label=r"$E^{(23)}$", marker=".")
        axs[i].plot(NUM_ELE, E31_errs, label=r"$E^{(31)}$", marker=".")
        axs[i].plot(NUM_ELE, Ep_errs, label=r"$E^{(+)}$", marker=".")
        axs[i].plot(NUM_ELE, Em_errs, label=r"$E^{(-)}$", marker=".")
        axs[i].plot(NUM_ELE, H3x3_0_errs, label=r"$Q^{(1)}$", marker=".")
        axs[i].plot(NUM_ELE, H3x3_1_errs, label=r"$Q^{(2)}$", marker=".")
        axs[i].plot(NUM_ELE, H3x3_2_errs, label=r"$Q^{(3)}$", marker=".")
        axs[i].set_ylim([0., 0.17])
        if i in [2, 3]:
    ...
    #fig.text(0.5, 0.90, "Average local relative error", ha='center', va='center')
