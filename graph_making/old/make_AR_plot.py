...
ASPECT_RATIOS = [2., 4., 6., 8., 10.]
FOLDER_NAMES = ["cp_le", "cp_qe", "lp_le", "lp_qe"]

def main():
    parser = argp.ArgumentParser(description="Creates 2x2 plot of error vs aspect ratio from a directory of vtk files with local relative error data")
    ...
    for i, fn in enumerate(FOLDER_NAMES):
        ...
        axs[i].plot(ASPECT_RATIOS, E12_errs, label=r"$E^{(12)}$", marker=".")
        axs[i].plot(ASPECT_RATIOS, E23_errs, label=r"$E^{(23)}$", marker=".")
        axs[i].plot(ASPECT_RATIOS, E31_errs, label=r"$E^{(31)}$", marker=".")
        axs[i].plot(ASPECT_RATIOS, Ep_errs, label=r"$E^{(+)}$", marker=".")
        axs[i].plot(ASPECT_RATIOS, Em_errs, label=r"$E^{(-)}$", marker=".")
        axs[i].plot(ASPECT_RATIOS, H3x3_0_errs, label=r"$Q^{(1)}$", marker=".")
        axs[i].plot(ASPECT_RATIOS, H3x3_1_errs, label=r"$Q^{(2)}$", marker=".")
        axs[i].plot(ASPECT_RATIOS, H3x3_2_errs, label=r"$Q^{(3)}$", marker=".")

        axs[i].set_xlabel("Aspect ratio")
        if i in [0, 2]:
            axs[i].set_ylabel("Average relative local error")
