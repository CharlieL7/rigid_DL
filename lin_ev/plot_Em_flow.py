...
def main():
    ...
    parser = argp.ArgumentParser(description="Em flow plot in 2D with ellipsoid insert")
    parser.add_argument("dims", nargs=3, type=float, help="ellipsoid dimensions")
    parser.add_argument(
        "-o",
        "--out_tag",
        help="tag to prepend to output plot",
        default="Em_flow_3D_quiver"
    )
    ...
    ax.quiver(x, y, z, vels[0], vels[1], vels[2], length=0.10, linewidth=1.00)
    ax.set_title(r"$E^{(-)}$")
    ax.set_xlabel(r"$x$")
    ax.set_ylabel(r"$y$")
    ax.set_zlabel(r"$z$")
