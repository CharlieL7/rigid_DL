...
def main():
    ...
    parser.add_argument("-o", "--out_tag", help="tag to prepend to output files", default="lp_qe_test")
    ...
    cells = [("triangle", quad_mesh.lin_faces)]
...
def eigval_err(mesh, C, eigval, E_d, E_c):
    v_in = efun.make_lp_qe_lin_vels(E_d, E_c, mesh)
...
def trans_eigval_err(mesh, C):
    ...
    v_in = np.zeros((mesh.lin_verts.shape[0], 1), dtype=v_cnst.dtype) + v_cnst
    eigval = -1.
    lambda_mat = eigval * np.identity(C.shape[0])
    g = np.dot((lambda_mat - C), v_in.flatten("C"))
    g = g.reshape(v_in.shape, order="C")
