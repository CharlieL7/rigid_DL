...
Doesn't work for spheres as is.
"""
...
kappa_vec = eig_vals.calc_3x3_eval(dims)
eigval_3x3 = np.real(-(1 + kappa_vec) / (kappa_vec -1))
m_iter = 0
print("eigenvalue:")
print(eigval_3x3)
...
u_3x3 = eig_helper.make_quad_eig_vels(pot_mesh, dims, kappa_vec)

psi = m_a.make_forcing_vec(pot_mesh, geo_mesh, np.ravel(u_3x3[m_iter]), f, l, mu) #eigenvector
q = np.linalg.solve(K + np.identity(3*num_nodes), (eigval_3x3[m_iter] + 1) * psi)
...
psi = m_a.make_forcing_vec(pot_mesh, geo_mesh, np.ravel(u_3x3[m_iter]), f, l, mu)
q = np.linalg.solve(K + np.identity(3 * num_nodes), (eigval_3x3[m_iter] + 1) * psi)
np.savetxt("q_vec_lp_le_E12.csv", np.reshape(q, (num_nodes, 3)), delimiter=",")
